    ...
    def __repr__(self):
        ...
        if self.type in ['KeyPress', 'KeyRelease']:
            ...
            if ord(chr) < ord("A"):
                chr = "?"
            return (
                "%s char='%s'%d keysym='%s' keycode=%d:%x state=%x keyinfo=%s"
                % (
                    self.type,
                    chr,
                    ord(self.char),
            ...
            return '%s x=%d y=%d state=%x' % (self.type, self.x, self.y, self.state)
        elif self.type == 'Configure':
            return '%s w=%d h=%d' % (self.type, self.width, self.height)
        elif self.type in ['FocusIn', 'FocusOut']:
            return self.type
        elif self.type == 'Menu':
            return '%s state=%x' % (self.type, self.state)
        else:
