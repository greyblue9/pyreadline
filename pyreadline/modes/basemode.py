...
####################################
    ...
    def rl_settings_to_string(self):
        ...
        out = [
            ...
            "%-20s: %s" % ("mark_directories", self.mark_directories),
            "%-20s: %s" % ("bell_style", self.bell_style),
            "------------- key bindings ------------",
        ...
        bindings.sort()
        out.extend(tablepat%(key) for key in bindings)
        return out
    ...
    def _bind_key(self, key, func):
        ...
        if not callable(func):
            print(f"Trying to bind non method to keystroke:{key},{func}")
            raise ReadlineError(
                f"Trying to bind non method to keystroke:{key},{func},{type(func)},{type(self._bind_key)}"
            )

        keyinfo = make_KeyPress_from_keydescr(key.lower()).tuple()
        log(f">>>{keyinfo} -> {func.__name__}<<<")
...
#completion commands    
    ...
    def _get_completions(self):
        ...
        if self.completer:
            ...
            log(f'text completions=<{list(map(ensure_unicode, completions))}>')
            ...
            if self.mark_directories == 'on':
                ...
                for f in completions:
                    if os.path.isdir(f):
                        ...
                        mc.append(f)
                completions = mc
            log(f'fnames=<{list(map(ensure_unicode, completions))}>')
        return completions
    ...
    def complete(self, e):    # (TAB)
        ...
        filename completion."""
        if completions := self._get_completions():
